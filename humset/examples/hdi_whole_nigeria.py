...
for title, chosen_metric in metrics.items():
    ...
    metric_regioncode_value = dict(zip(region_codes, metric_per_region))
    r, pval = stats.pearsonr(region_hdis, metric_per_region)
    correlation.append((r, pval))
    regs = sbc.get_shapes('Nigeria')
    sbc.plot_whole_country(regions=regs, metric_title=title, metric_per_region=metric_regioncode_value)
...
# for metric, regcode_val in metric_regioncode_value.items():
